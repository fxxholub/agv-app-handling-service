    ...
    ) : ISessionExecutorService
{
    ...
    /// - Task Result.Error()
    /// </returns>
    public async Task<Result> StartSessionAndReserveConnection(string connectionId, HandlingMode handlingMode, bool mapping)
    {
        ...
        var createdResult = await createSessionService.CreateSession(
            handlingMode,
            HandlingModeLifespanMap(handlingMode),
            mapping
            );
